...
int life(long oldWorld[N][N], long newWorld[N][N]) {
  ...
  // clear the new world
  // Change 1: change the access order from col-order to row-order
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++) {
      newWorld[i][j] =  0;
    }
  ...
  // Count the cells to the top left
  // Change 4: col-major to row-major
  for (i = 0; i < N; i++) {
    //col = (i -1 + N) % N;
    row = (i - 1 + N) % N; 
    for (j = 0; j < N; j++) {
  ...
  // Count the cells immediately above
  // Change 3: change the order from col-major to row-major
for (i = 0; i < N; i++) {
    row = (i - 1 + N) % N;
    for (j = 0; j < N; j++) {
 ...
 // Count the cells to the immediate left
 // Change 2: change the count order from col-major to row-major
  for (i = 0; i < N; i++) {
    for (j = 0; j < N; j++) {
       col = (j - 1 + N) % N;;
...
// Count the cells to the bottom left
 // Change4: col-major to row-major
 for (i = 0; i < N; i++) {
     col = (i - 1 + N) % N;
   for (j = 0; j < N; j++) {
      row = (j + 1 + N ) % N;
